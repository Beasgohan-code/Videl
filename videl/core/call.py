...
from videl.core.queue import clear_queue, peek_current, pop_current, queue_size
from videl.utils.helpers import delete_file
from videl.utils.rich_ui import rich_esc, rich_heading, rich_kv_table, rich_note, rich_send
...
async def on_stream_end(_: object, update: StreamEnded) -> None:
    ...
    # Loop modes
    try:
        from videl.core.player import get_loop
        from videl.core.queue import add_to_queue, peek_current as _peek
        mode = get_loop(chat_id)
        current = _peek(chat_id)
        if mode == "one" and current:
            # Replay same track without popping
            from videl.core.player import play_song
            msg = await rich_send(
                bot, chat_id,
                rich_heading("🔁 ʟᴏᴏᴘ ᴏɴᴇ", level=3)
                + rich_kv_table([("ᴛɪᴛʟᴇ", f"<code>{rich_esc(current.get('title', '?'))}</code>")]),
            )
            await play_song(chat_id, msg, current)
            return
        if mode == "all" and current:
            # Move finished song to end of queue
            done = pop_current(chat_id)
            if done:
                # don't delete file if re-queued as local — prefer re-resolve
                add_to_queue(chat_id, {k: v for k, v in done.items() if k != "file_path"})
        else:
            done = pop_current(chat_id)
            if done:
        ...
        LOGGER.warning(f"[loop] {loop_err}")
        done = pop_current(chat_id)
        if done:
            try:
                delete_file(done.get("file_path", ""))
    ...
    try:
        from videl.core.autoplay import is_autoplay, maybe_refetch
    ...
    if nxt:
        ...
        try:
            from videl.core.autoplay import (
                is_autoplay,
                _autoplay_fetching,
            )
            ...
            if is_autoplay(chat_id):
                ...
                from videl.core.autoplay import maybe_refetch
